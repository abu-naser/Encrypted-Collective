...
/* Added by abu naser */
EVP_AEAD_CTX *global_ctx;
int global_pipeline_size = 65536;
/* end of add */
...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    MPID_Comm_get_ptr(MPI_COMM_WORLD, comm_ptr);
    shmem_leaders=1;
    char *s_value, *o_value, *t_value, *c_value, *ob_value, *a_value, *rl_value;
    ...
    /* This part need to be modify when it will merge with allgather, right now use this */
    if (allocated_shmem == 0)
    {
        if ((t_value = getenv("SCATTER_SHRD_MEM")) != NULL)
        {
            int alg = (atoi(t_value));
            if (alg == 1)
            {
    ...
    if ((t_value = getenv("PIPELINE_SIZE")) != NULL)
    {
        global_pipeline_size = (atoi(t_value));
    }
    /* end of add */
...
int init_shmem(){
    ...
    //TODO: Allocate Shmem
    size_t shmem_size = (comm_ptr->local_size) * 4 * 128 *1024;
    size_t ciphertext_shmem_size = (comm_ptr->local_size) * (128 * 1024 * 4 + 16 + 12);
    shmem_key = 20202;
    ciphertext_shmem_key = 31313;
