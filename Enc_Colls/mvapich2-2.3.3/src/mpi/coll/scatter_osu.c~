                            ...
                            int root, MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    int pipeline_size = global_pipeline_size; // define the pipeline size here
    int segments_no;
                            ...
                            int root, MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    */
    ...
    int leader_size = local_size; 
    unsigned char * temp_buffer;
    int pipeline_size = global_pipeline_size;
    unsigned int index_seg;
    int segments_no;
    int j;
...
/*
*
* no share memory and multi-leader
*
*/
#undef FUNCNAME
...
#endif
    if(security_approach == 201) // call shared memory implementation
   {
       int ret = MPIR_Scatter_MV2_Direct_shmem(sendbuf, sendcnt, sendtype, recvbuf, recvcnt,recvtype,root, comm_ptr,errflag);
       return ret;
   }         
    else if(security_approach == 202) // call shared memory implementation, h2
   {
       int ret = MPIR_Scatter_MV2_Direct_HS2(sendbuf, sendcnt, sendtype, recvbuf, recvcnt,recvtype,root, comm_ptr,errflag);
       return ret;
   }
     else if(security_approach == 203) // call shared memory implementation with ML
   {
       int ret = MPIR_Scatter_MV2_Direct_CHS(sendbuf, sendcnt, sendtype, recvbuf, recvcnt,recvtype,root, comm_ptr,errflag);
       return ret;
   }
   else  if(security_approach == 204) // call no shared memory + ML implementation
   {
       int ret = MPIR_Scatter_MV2_Direct_no_shmem(sendbuf, sendcnt, sendtype, recvbuf, recvcnt,recvtype,root, comm_ptr,errflag);
       return ret;
   }
    else  if(security_approach == 205) // call no shared memory + ML + pipeline implementation       
   {
         int ret = MPIR_Scatter_MV2_Direct_no_shmem_pipeline(sendbuf, sendcnt, sendtype, recvbuf, recvcnt,recvtype,root, comm_ptr,errflag);
        return ret;
   } 
    else  if(security_approach == 206) // call no shared memory + ML + intra-div-conq implementation       
   {
         int ret = MPIR_Scatter_MV2_Direct_no_shmem_intra_RR(sendbuf, sendcnt, sendtype, recvbuf, recvcnt,recvtype,root, comm_ptr,errflag);
        return ret;
   }
    else  if(security_approach == 207) // call HS2-ML + pipeline     
   {
         int ret = MPIR_Scatter_MV2_Direct_HS2_ML_Pipeline(sendbuf, sendcnt, sendtype, recvbuf, recvcnt,recvtype,root, comm_ptr,errflag);
        return ret;
   }
    else  if(security_approach == 208) // call HS2-ML Unencrypted 
   {
         int ret = MPIR_Scatter_MV2_Direct_HS2_ML_UNENC(sendbuf, sendcnt, sendtype, recvbuf, recvcnt,recvtype,root, comm_ptr,errflag);
        return ret;
   }
    MPIR_TIMER_START(coll,scatter,direct);
    int rank, comm_size;
    ...
    if (((comm_ptr->comm_kind == MPID_INTRACOMM) && (root == rank)) ||
        ...
        for (i = 0; i < comm_size; i++) {
            ...
            if ((comm_ptr->comm_kind == MPID_INTRACOMM) && (i == rank)) {
                ...
                MPIR_PVAR_INC(scatter, direct, send, sendcnt, sendtype);
                    ...
                    if(security_approach == 200 && rank_node_id != dest_node_id)
                    {
        ...
        if (recvcnt) {
            ...
            if(security_approach == 200 && rank_node_id != root_node_id)
            {
                int j = 0;
                int next = 0;
                int dest = 0;
                                      ...
                                      MPIR_Errflag_t *errflag)
{
  ...
  if(security_approach == 201) // call shared memory implementation
   ...
   else if(security_approach == 202) // call shared memory implementation
   {
       int ret = MPIR_Scatter_MV2_Direct_HS2(sendbuf, sendcnt, sendtype, recvbuf, recvcnt,recvtype,root, comm_ptr,errflag);
       return ret;
   }
   else if(security_approach == 203) // call shared memory implementation
   {
       int ret = MPIR_Scatter_MV2_Direct_CHS(sendbuf, sendcnt, sendtype, recvbuf, recvcnt,recvtype,root, comm_ptr,errflag);
       return ret;
   }
