                          ...
                          MPIR_Errflag_t *errflag)
{
    ...
    rank = comm_ptr->rank;
    if(rank==0 && recvcount==2048)
	printf("Hello from RD\n");
    ...
    if(recvcount==2048)
	printf("%d Goodbye, send_req_idx=%d\n", rank, send_req_idx);
    ...
    *************************************************/
	...
	if(recvcount==2048)
		...
		case 3:
            ...
            MPIR_PVAR_INC(allgather, ring, recv, recvcount, recvtype);
	    if(recvcount==2048)
		printf("%d - %d is going to send %d to %d and recv %d from %d\n", i-1, rank, j, right, jnext, left);
            mpi_errno = MPIC_Sendrecv(((char *) recvbuf +
                                        j * recvcount * recvtype_extent),
                                            recvcount, recvtype, right,
                                     MPIR_ALLGATHER_TAG,
