                            ...
                            int root, MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    rank = comm_ptr->rank;
    if(rank==0)
	printf("Oh, MPIR_Bcast_binomial_MV2\n");
                                 ...
                                 int is_contig, int is_homogeneous, MPIR_Errflag_t *errflag)
{
    ...
    relative_rank = (rank >= root) ? rank - root : rank - root + comm_size;
    if (rank==0)
	printf("Oh, scatter_for_bcast_MV2\n");
                                              ...
                                              MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    relative_rank = (rank >= root) ? rank - root : rank - root + comm_size;
    if (rank==0)
	printf("Oh, MPIR_Bcast_scatter_doubling_allgather_MV2\n");
    /* If there is only one process, return */
    if (comm_size == 1)
        goto fn_exit;
...
{   
    ...
    MPID_Datatype_get_extent_macro(datatype, extent);
    if(rank==0)
	printf("Oh, MPIR_Pipelined_Bcast_Zcpy_MV2\n");
    int ii = comm_ptr->dev.ch.rank_list_index;
    printf("%d -> %d \n", rank, ii);
    MPIU_Assert(mv2_enable_zcpy_bcast==1 && mv2_use_slot_shmem_coll==1);
    local_rank = shmem_commptr->rank;
    nbytes = count*extent; 
