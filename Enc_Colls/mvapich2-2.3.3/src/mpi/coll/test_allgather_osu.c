                          ...
                          MPIR_Errflag_t *errflag)
{
    ...
    rank = comm_ptr->rank;
    if(rank==0)
	printf("%d- MPIR_Allgather_RD_MV2\n", recvcount);

    if(recvcount==1){
	if(rank==0){
	    printf("is_global_block=%d, is_blocked=%d\n", comm_ptr->dev.ch.is_global_block, comm_ptr->dev.ch.is_blocked);
	}
	MPID_Node_id_t node_id;    
	MPID_Get_node_id(comm_ptr, rank, &node_id);
	int my_node = node_id;
	printf("%d is on node %d\n", rank, my_node);
    }
                             ...
                             MPIR_Errflag_t *errflag)
{
    ...
    rank = comm_ptr->rank;
    if(rank==0)
	printf("%d- MPIR_Allgather_Bruck_MV2\n", recvcount);
    /************* For MPIC_Sendrecv_Plus ************
                            ...
                            MPIR_Errflag_t *errflag)
{
    ...
    rank = comm_ptr->rank;
    if(rank==0)
    	printf("%d- MPIR_Allgather_Ring_MV2\n", recvcount);
    /************* For MPIC_Sendrecv_Plus ***************
    MPID_Request *send_req_ptr[comm_size-1];
    int p;
    for(p=0; p<comm_size-1; ++p){
